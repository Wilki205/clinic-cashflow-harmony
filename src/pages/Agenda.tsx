...
interface Appointment {
  ...
  time: string;
  procedure: string;
  status: "pending" | "confirmed" | "completed" | "absent" | "rescheduled";
  valor?: number;
}
...
export default function Agenda() {
  ...
  const loadAppointments = async () => {
    try {
      ...
      const formatted: Appointment[] = data.map((item: any) => ({
        ...
        date: item.data_agendamento?.split("T")[0] || item.data_agendamento,
        time: item.horario_agendamento?.slice(0, 5) || item.horario_agendamento,
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/agendamentos/${selectedApp.id}`, {
        method: "DELETE",
      });
    ...
    try {
      ...
      const res = await fetch(`/api/agendamentos/${selectedApp.id}`, {
        ...
        body: JSON.stringify({
          ...
          horario: rescheduleData.time,
          procedimento: selectedApp.procedure,
          valor: selectedApp.valor || 0,
          status: "rescheduled",
        }),
      });
      ...
      if (!res.ok) {
        const text = await res.text();
        console.error("Erro da API:", text);
        throw new Error("Erro ao remarcar consulta");
      }
    ...
    switch (status) {
      ...
      default:
        return {
          color: "bg-amber-400",
          text: "Pendente",
          icon: <CircleDashed className="h-3.5 w-3.5 animate-pulse" />,
        ...
        {appointments.length === 0 ? (
          <Card className="border-dashed border-2 bg-slate-50/50 p-10 text-center">
            ...
            <p className="text-slate-400 font-medium">
              {loading ? "Carregando agenda..." : "Nenhum agendamento para mostrar."}
            </p>
          </Card>
        ) : (
                ...
                <div className="flex">
                  ...
                  <CardContent className="flex-1 p-5 flex items-center justify-between bg-white">
                    <div className="flex items-center gap-6">
                      ...
                      <div>
                        <p className="font-bold text-slate-800 text-lg group-hover:text-primary transition-colors">
                          {app.patient_name}
                        </p>
                        <span className="text-[10px] bg-slate-100 px-2 py-0.5 rounded font-bold text-slate-500 uppercase tracking-wider">
                          {app.procedure}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      <div
                        className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold border ${config.style}`}
                      >
...
>
  ...
  {app.status !== "completed" && (
    <DropdownMenuItem
      onClick={() => openDeleteAlert(app)}
      className="text-rose-600 cursor-pointer border-t mt-1 font-bold"
    >
      ...
      <Dialog open={isRescheduleModalOpen} onOpenChange={setIsRescheduleModalOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold">Remarcar Consulta</DialogTitle>
          </DialogHeader>
        ...
        <AlertDialogContent className="border-none shadow-2xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-slate-600 text-base">
              Tem certeza que deseja remover o horário de{" "}
              <strong>{selectedApp?.patient_name}</strong>?
            </AlertDialogDescription>
          </AlertDialogHeader>

          <AlertDialogFooter className="mt-4 gap-2">
            <AlertDialogCancel className="border-slate-200 text-slate-500 hover:bg-slate-50">
              Cancelar
            </AlertDialogCancel>
            <AlertDialogAction
